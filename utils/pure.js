...
export const cleanStringAndRemoveSpaces = R.compose(
	R.replace(/ /g, ""),
	cleanString
);
...
export const includesElOf = R.curry((list1, list2) =>
	R.any((el) => R.includes(el, list2), list1)
);

// true if list2 has element that appears in list1 else false
// includesElOf :: [a] -> [a] -> Bool
export const includesElOfAll = R.curry((list1, list2) =>
	R.all((el) => R.includes(el, list2), list1)
);
...
export const partiallyIncludesElOf = R.curry((list1, list2) =>
	R.any((el2) => R.any(R.includes(el2), list1), list2)
);
...
export const transformToResources = (categories) => {
	...
	return R.map(
		(category) =>
			R.over(
				resourcesLens,
				R.map(addCleanTitleAndPath(category.slug)),
				category
			),
		categories
	);
